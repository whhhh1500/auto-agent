...
| M33 Unicode-token candidate | Repo-external design/evidence is `D:\cc\auto_agent\audit\rag-unicode-tokenization-20260908\v47-unicode-delimiter-design.md`; it limits token delimiters to `，、。；：！？（）［］【】「」『』《》〈〉`, preserving curly quotes, dashes, ASCII URL/email/code and `l’été`. | Fixed qrels after simulated v47 rebuild improve only the CJK lexical miss: Recall@5 `5/7→6/7`, precision `5/10→6/11`, macro MRR `0.5→0.7`; semantic miss remains. | **pending implementation, separate from HTTP**: legacy persisted tokens retain `5/7`/`0.5`; any implementation needs atomic v47 token rebuild plus marker and old-binary refusal of DB47. | This is not general Chinese segmentation or a claim about production traffic; do not mix it into the HTTP classifier commit. |
