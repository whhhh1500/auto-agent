...
func TestSQLSchemaV41MigratesAuthorizationEpoch(t *testing.T) {
	...
	if err := db.QueryRowContext(ctx, "SELECT value FROM store_meta WHERE key = 'schema_version'").Scan(&freshVersion); err != nil || freshVersion != "43" {
		t.Fatalf("fresh schema version=%q err=%v, want 43", freshVersion, err)
	}
	...
	var version string
	if err := db.QueryRowContext(ctx, "SELECT value FROM store_meta WHERE key = 'schema_version'").Scan(&version); err != nil || version != "43" {
		t.Fatalf("schema version=%q err=%v, want 43", version, err)
	}
}
