...
func TestSQLSchemaV41MigratesAuthorizationEpoch(t *testing.T) {
	...
	if err := db.QueryRowContext(ctx, "SELECT value FROM store_meta WHERE key = 'schema_version'").Scan(&freshVersion); err != nil || freshVersion != "44" {
		t.Fatalf("fresh schema version=%q err=%v, want 44", freshVersion, err)
	}
	...
	var version string
	if err := db.QueryRowContext(ctx, "SELECT value FROM store_meta WHERE key = 'schema_version'").Scan(&version); err != nil || version != "44" {
		t.Fatalf("schema version=%q err=%v, want 44", version, err)
	}
}
