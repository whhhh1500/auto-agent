...
func serialRollingSummary(t *testing.T, model *serialAcceptanceModel) {
	...
	for _, enabled := range []bool{false, true} {
		...
		for turn, name := range names {
			...
			serialSeedSummaryHistory(t, api, sessionID, turn, names, markers)
			before := model.calls
			evidence := serialRun(t, api, sessionID, "What is the verification code for warehouse "+name+"? Return only that code.", markers[turn])
			if model.calls-before != 1 || len(evidence.calls) != 0 || strings.TrimSpace(evidence.answer) != markers[turn] {
			...
			if factSources != 1 || (enabled && (summaryMessages != 1 || len(sent) > 120 || len(summaries[0].Summary) <= 1024)) {
		...
		arms = append(arms, map[string]any{"summary_enabled": enabled, "run_ids": runs, "input_tokens": input, "output_tokens": output, "elapsed_ms": elapsed, "requests": requests, "projections": projections, "service_pool_replacements": 2})
		serialWriteAudit(t, "rolling_summary_comparison", map[string]any{"policy": "extractive 120/60; mechanical compactor disabled in both arms", "seed_turns": []int{64, 32, 32}, "distinct_facts": 3, "arms": arms})
		t.Logf("rolling_summary enabled=%t model_calls=%d input_tokens=%d output_tokens=%d elapsed_ms=%d correct_answers=3 restores=2", enabled, len(requests), input, output, elapsed)
	}
}

func serialSeedSummaryHistory(t *testing.T, api *serialAuditedAPI, sessionID string, batch int, names, markers []string) {
	t.Helper()
