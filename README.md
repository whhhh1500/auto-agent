...
`CapabilityResolver.Resolve` walks the scope hierarchy and creates an immutable `CapabilitySnapshot`. The snapshot is the only capability set used by a run. Its ID fingerprints the frozen scope, subject, and selected capability definitions; it is not a credential or a complete-principal authorization proof.
...
The current SQL schema is v47. Earlier memory/RAG tag and token projections are rebuildable derived indexes over canonical entries/documents. Later additions include artifact-migration state and journal records (v36), experimental Graph checkpoint/transition facts (v37), encrypted tenant-scoped notification targets (v38), generation-fenced Graph segment leases (v39), approval/stale-run lookup indexes (v40), immutable Graph checkpoint-version history (v41), a separate durable `authorization_epoch` control-plane row (v42), immutable completed-tool-result sidecars (v43), generation-scoped native queued effect-admission witnesses (v44), one-attempt native queued main-model admissions (v45), and immutable canonical model-outcome delivery evidence (v46). V47 atomically rebuilds the derived RAG token/tag projection from canonical documents for the fixed CJK/fullwidth delimiter tokenizer; it adds no table. The v41 upgrade atomically backfills only each v40 head as a `migration_floor` and installs a write fence, so pre-v41 head-only writers fail rather than split a current head from its history. The v42 epoch is intentionally separate from `control_revision`: it advances with built-in SQL release, canary, persistent binding, and active-account authorization mutations, while control synchronizers keep using their narrower revision. A v43 sidecar is SQL-local historical delivery evidence for one canonical `tool/result`; it does not add `run/resume`, grant continuation, consume acknowledgement state, or make automatic recovery available. Its referenced journal proof is retained fail-safe until a future terminal/superseded-sidecar GC protocol exists. A v44 witness atomically pairs journal Begin with the epoch, live queued fence, durable composition, and tool/call tail for one queue generation; it does not prove provider completion, current account state, or recovery eligibility. A v45 model attempt binds one durable `step/start`, exact non-secret `ModelCallRequest`, epoch, queue generation, session lease, and model contract before the external model request; an existing attempt forbids provider replay, and this first slice stores no prompt digest or canonical assistant outcome. V46 atomically binds a Core-validated assistant/message plus model usage suffix to its v45 attempt and fenced Session tip; it is historical outcome delivery only and still does not authorize continuation or provider replay. `SQLSessionStore.PruneNativeQueuedModelInvocations` can remove an old v45/v46 pair only after strict proof validation and either a terminal run or a durable Session successor; attempts without an outcome remain permanent replay fences. The store refuses a database newer than this binary; use the v47 maintenance procedure before upgrading a shared database. The complete versioned chain is recorded in the public inventory rather than duplicated as stale migration prose here.

Event payloads are JSONL TEXT in every durable store, so sessions migrate between backends by copying data. Loads restore only the committed prefix and never append repair events; crash-tail repair is an explicit execution-owner operation. The SQL and S3 restore paths reject a prefix shorter than the committed event count or an invalid committed version, returning an error rather than silently rebuilding or replaying history.
...
Schema versioning: the SQL schema is versioned in `store_meta` (currently v47). Higher versions are inspected and refused before current DDL runs. Historical migrations through v35 retain the original core/session/control-plane records. v36 adds the resource-object migration slot and metadata-only mutation journal; v37 adds experimental Graph checkpoint and append-only transition facts; v38 adds encrypted tenant-scoped notification targets; v39 adds generation-fenced Graph segment leases; v40 adds approval-by-run and stale-run recovery indexes; v41 adds immutable Graph checkpoint-version history; v42 adds the separate `authorization_epoch` row; v43 adds the SQL-local completed-tool-result sidecar; v44 adds generation-scoped native queued tool-effect admission witnesses; v45 adds one-attempt native queued main-model admission rows; v46 adds immutable canonical model-outcome evidence; and v47 atomically rebuilds the derived RAG token/tag projection under the fixed CJK/fullwidth delimiter tokenizer. The v41 upgrade is one transaction: it backfills only the current v40 head as `migration_floor`, installs the head-write protocol fence, and records the v41 marker together; a v40 head-only writer is rejected after the fence instead of silently splitting history. V43 atomically binds one canonical `tool/result` and immutable historical composition evidence to the current fenced SQL transaction; it is not a continuation grant, acknowledgement protocol, or automatic recovery coordinator. The v42 epoch is an audit/fencing input for future SQL recovery work, not a grant for arbitrary external or in-memory authorization sources. The detailed historical chain is maintained in the machine-readable public inventory; upgrades from v22 still rebuild Assignment variants from canonical Session events before recording the current version.
