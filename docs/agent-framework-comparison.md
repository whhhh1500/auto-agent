...
**对当前项目的投入建议：** 保留已有 Go 服务治理与小接口方向。[工具预算](performance/2026-09-06-context-budget-optimization.md)、[WASM 资源与编译复用](performance/2026-09-06-wasm-resource-and-cache.md)、[工具披露与整段会话成本](performance/2026-09-06-tool-disclosure.md)已有具体修复与实测。披露在 24 工具样本节省 token，在 4 工具样本更贵且两组都更慢，不能作为统一默认优化。接下来继续长任务恢复、更精确的协议成本模型与跨框架对等任务集；复杂编排与 RAG 通过原型决定复用或自研，避免用一个总分掩盖各模块的不同成熟度。
