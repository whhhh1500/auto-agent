...
func TestSQLSchemaV41MigratesAuthorizationEpoch(t *testing.T) {
	...
	if err := db.QueryRowContext(ctx, "SELECT value FROM store_meta WHERE key = 'schema_version'").Scan(&freshVersion); err != nil || freshVersion != strconv.Itoa(SQLSchemaVersion) {
		t.Fatalf("fresh schema version=%q err=%v, want %d", freshVersion, err, SQLSchemaVersion)
	}
	...
	var version string
	if err := db.QueryRowContext(ctx, "SELECT value FROM store_meta WHERE key = 'schema_version'").Scan(&version); err != nil || version != strconv.Itoa(SQLSchemaVersion) {
		t.Fatalf("schema version=%q err=%v, want %d", version, err, SQLSchemaVersion)
	}
}
