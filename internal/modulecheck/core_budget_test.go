...
func productionCoreFiles(coreDir string) ([]string, error) {
	...
	for _, entry := range entries {
		...
		if isGeneratedGo(entry.Name(), contents) {
			continue
		}
...
func isGeneratedGo(name string, contents []byte) bool {
	if strings.HasSuffix(name, "_generated.go") || strings.Contains(name, "generated") {
		return true
	}
	for _, line := range bytes.Split(contents, []byte{'\n'}) {
		line = bytes.TrimSpace(line)
		if bytes.HasPrefix(line, []byte("// Code generated ")) && bytes.Contains(line, []byte(" DO NOT EDIT.")) {
			return true
		}
	}
	return false
